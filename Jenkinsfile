pipeline {
    agent any // 어떤 에이전트(실행 서버)에서도 실행 가능
    ...
    environment {
        ...
        REMOTE_HOST = "15.165.203.181" // 원격(spring) 서버 public IP

        REMOTE_DIR = "/home/ec2-user/deploy" // 원격 서버에 파일 복사할 경로
        SSH_CREDENTIALS_ID = "86ca4d17-61e0-48c6-97e6-a79998aacf31"
    }
    ...
    stages {
        ...
        stage('Maven Build') {
            steps {
                // 테스트는 건너뛰고 Maven 빌드
                sh 'mvn clean package -DskipTests' // jar 파일 생성
        ...
        stage('Copy to Remote Server') {
            steps {
                // Jenkins가 원격 서버에 ssh 접속할 수 있도록 ssh agant 사용
                sshagent (credentials: [env.SSH_CREDENTIALS_ID]) {
                    // 원격 서버에 배포 디렉토리 생성 (없으면 새로 만들고 있음)
